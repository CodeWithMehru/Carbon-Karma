...
export default function LoginPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      ...
      if (result?.error) {
        ...
        toast({ title: 'Welcome Back!', description: 'Successfully signed in to Carbon Karma.', type: 'success' });
  ...
  return (
    <main id="main-content" className="flex-1 flex items-center justify-center min-h-screen relative overflow-hidden bg-gradient-to-br from-emerald-50 via-[#f0fdf4] to-amber-50 py-12 px-4">
      ...
      >
        ...
        <div className="text-center mb-8">
          <Link href="/" className="inline-flex items-center gap-2 text-2xl font-bold font-heading text-emerald-800 focus:outline-none focus:ring-2 focus:ring-emerald-500 rounded-md p-1">
            <Leaf className="h-7 w-7 text-emerald-600" />
            <span>Carbon Karma</span>
          </Link>
          <p className="mt-1.5 text-sm text-emerald-700">Gamifying carbon reduction for a greener India</p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-emerald-100 overflow-hidden">
          ...
          <div className="px-8 pt-8 pb-6 border-b border-emerald-50">
            <h1 className="text-2xl font-bold text-emerald-950 font-heading text-center">Welcome back</h1>
            <p className="text-sm text-emerald-600 text-center mt-1">Sign in to access your Karma dashboard</p>
          </div>

          <div className="px-8 py-6 space-y-5">
            {/* Error Alert */}
            {errorMsg && (
              <div role="alert" className="flex items-start gap-2.5 rounded-xl bg-red-50 px-4 py-3 text-sm text-red-800 border border-red-200">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-xs text-emerald-500 font-medium uppercase tracking-wide">or with email</span>
              <div className="flex-1 h-px bg-emerald-100" />
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-1.5">
                <div className="flex items-center justify-between">
                  ...
                  <Link href="/forgot-password" className="text-xs text-emerald-600 hover:text-emerald-800 hover:underline transition-colors">
                    Forgot password?
                  </Link>
                </div>
                <div className="relative">
          ...
          <div className="px-8 py-5 bg-emerald-50/60 border-t border-emerald-100 text-center">
            <span className="text-sm text-emerald-700">
              ...
              <Link href="/signup" className="font-bold text-emerald-700 hover:text-emerald-900 hover:underline transition-colors">
                Sign Up Free
              </Link>
            </span>
          </div>
