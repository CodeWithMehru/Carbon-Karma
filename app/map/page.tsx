...
import { Map as MapIcon, Trophy, Sprout, Users, ArrowUpRight } from 'lucide-react';
...
export default async function MapPage() {
  ...
  const { data: dbCities, error } = await (supabase as any)
  ...
  const seedCities: CitySummary[] = [
    { city: 'Mumbai', state: 'Maharashtra', user_count: 128, total_karma: 12540, total_kg_saved: 420.5 },
    { city: 'Bengaluru', state: 'Karnataka', user_count: 96, total_karma: 9810, total_kg_saved: 310.8 },
    { city: 'Delhi', state: 'Delhi', user_count: 142, total_karma: 8430, total_kg_saved: 290.4 },
    { city: 'Pune', state: 'Maharashtra', user_count: 64, total_karma: 5120, total_kg_saved: 180.2 },
    { city: 'Srinagar', state: 'Jammu & Kashmir', user_count: 42, total_karma: 3950, total_kg_saved: 140.6 },
  ];
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <Card className="glass border-emerald-100 shadow-sm">
          <CardHeader className="pb-2">
            <CardDescription className="font-semibold text-emerald-800">Total Community Offset</CardDescription>
            <CardTitle className="text-3xl font-bold text-emerald-950 flex items-baseline gap-1.5">
              {totalOffset.toLocaleString(undefined, { minimumFractionDigits: 1, maximumFractionDigits: 1 })}
        ...
        <Card className="glass border-emerald-100 shadow-sm">
          <CardHeader className="pb-2">
            <CardDescription className="font-semibold text-emerald-800">Active Changemakers</CardDescription>
            <CardTitle className="text-3xl font-bold text-emerald-950 flex items-baseline gap-1.5">
        ...
        <Card className="glass border-emerald-100 shadow-sm">
          <CardHeader className="pb-2">
            <CardDescription className="font-semibold text-emerald-800">Indian States Reached</CardDescription>
            <CardTitle className="text-3xl font-bold text-emerald-950 flex items-baseline gap-1.5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <Card className="lg:col-span-2 glass border-emerald-100 shadow-sm flex flex-col justify-between overflow-hidden relative min-h-[450px]">
          <CardHeader>
            ...
            <CardDescription>Visualizing concentration of eco-activities across regions</CardDescription>
          </CardHeader>
          <CardContent className="flex-1 flex items-center justify-center p-8 bg-emerald-50/10 relative">
            ...
            <div className="relative w-full max-w-sm aspect-[4/5] border border-emerald-100/50 bg-emerald-50/20 rounded-2xl flex items-center justify-center shadow-inner overflow-hidden">
              <span className="text-xs font-semibold text-emerald-800/60 uppercase tracking-wider">India Impact Grid</span>
              ...
              <span className="absolute top-[18%] left-[45%] h-4 w-4 bg-emerald-600 rounded-full border-2 border-white shadow-sm" title="Srinagar" />

              <span className="absolute top-[40%] left-[30%] h-8 w-8 bg-emerald-500/10 rounded-full animate-ping" style={{ animationDelay: '0.5s' }} />
              <span className="absolute top-[40%] left-[30%] h-4 w-4 bg-emerald-600 rounded-full border-2 border-white shadow-sm" title="Delhi" />

              <span className="absolute top-[68%] left-[28%] h-8 w-8 bg-emerald-500/10 rounded-full animate-ping" style={{ animationDelay: '1s' }} />
              <span className="absolute top-[68%] left-[28%] h-4 w-4 bg-emerald-600 rounded-full border-2 border-white shadow-sm" title="Mumbai" />

              <span className="absolute top-[72%] left-[32%] h-8 w-8 bg-emerald-500/10 rounded-full animate-ping" style={{ animationDelay: '1.5s' }} />
              <span className="absolute top-[72%] left-[32%] h-4 w-4 bg-emerald-600 rounded-full border-2 border-white shadow-sm" title="Pune" />

              <span className="absolute top-[82%] left-[38%] h-8 w-8 bg-emerald-500/10 rounded-full animate-ping" style={{ animationDelay: '2s' }} />
              <span className="absolute top-[82%] left-[38%] h-4 w-4 bg-emerald-600 rounded-full border-2 border-white shadow-sm" title="Bengaluru" />
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-0 flex-1 overflow-auto">
            <div className="divide-y divide-emerald-100/50">
              {mergedCities.map((city, idx) => (
                <div key={city.city} className="flex items-center justify-between p-4 hover:bg-emerald-50/20 transition-colors">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-[#4a6a4a]">{city.state}</p>
                    </div>
                  </div>
                  
                  <div className="text-right">
                    <p className="text-xs font-bold text-emerald-800">{city.total_karma.toLocaleString()} Karma</p>
