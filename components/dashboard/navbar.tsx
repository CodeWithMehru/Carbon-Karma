...
import { Leaf, LayoutDashboard, Target, Users, Camera, Map } from 'lucide-react';
    ...
    <header className="sticky top-0 z-40 w-full glass border-b border-border/50">
      <div className="container mx-auto px-4 h-16 flex items-center justify-between">
        <div className="flex items-center gap-8">
          <Link href="/dashboard" className="flex items-center gap-2 text-emerald-800 font-bold font-heading text-lg">
          ...
          <nav className="hidden md:flex items-center gap-6">
            {NAV_ITEMS.map((item) => {
              ...
              return (
                ...
                >
                  <Icon className={cn("h-4 w-4", isActive ? "text-emerald-600" : "text-emerald-600/50")} />
                  {item.label}
                </Link>
              )
            })}
        ...
        <div className="flex items-center gap-4">
          <form action={signOut}>
            <Button type="submit" variant="ghost" size="sm" className="text-[#4a6a4a] hover:text-red-600">
              Sign Out
            </Button>
          </form>
        </div>
      ...
      <div className="md:hidden flex overflow-x-auto py-2 px-4 gap-4 border-t border-border/50 bg-white/50 backdrop-blur-md">
        {NAV_ITEMS.map((item) => {
          ...
          return (
            ...
            >
              <Icon className={cn("h-5 w-5", isActive ? "text-emerald-600" : "text-emerald-600/50")} />
              {item.label}
            </Link>
          )
        })}
