...
// ─── AI Insight Schema ───────────────────────────────────────────────────────
// Validates the structure of a single personalized insight, whether produced by
// the deterministic engine or the Gemini-powered route.

export const insightSchema = z.object({
  icon: z.string().min(1).max(8),
  title: z.string().min(1).max(60),
  desc: z.string().min(1).max(320),
  impact: z.string().min(1).max(60),
  category: z.string().max(40).optional(),
});

export const insightsResultSchema = z.array(insightSchema).min(1).max(6);

export type InsightInput = z.infer<typeof insightSchema>;

// ─── Gemini Vision Receipt Schema ────────────────────────────────────────────
// Validates the structured JSON returned by the Gemini receipt-parsing prompt.
// Numbers are coerced because vision models occasionally emit numeric strings.

export const geminiReceiptItemSchema = z.object({
  name: z.string().min(1).max(200),
  category: z.enum(['food', 'transport', 'energy', 'shopping', 'other']),
  quantity: z.coerce.number().nonnegative(),
  unit: z.string().min(1).max(40),
  price_inr: z.coerce.number().nonnegative(),
  estimated_kg_co2: z.coerce.number().nonnegative(),
  sustainability_factor: z.enum(['high_carbon', 'neutral', 'sustainable']),
});

export const geminiReceiptSchema = z.object({
  store_name: z.string().min(1).max(200),
  total_inr: z.coerce.number().nonnegative(),
  items: z.array(geminiReceiptItemSchema).min(1).max(100),
  overall_sustainability_score: z.coerce.number().min(0).max(100),
  feedback_message: z.string().min(1).max(500),
});
